---
layout: default
title: Contact
description: Contact salvaged.work
permalink: /contact/
last_modified_at: 2026-08-11
---

<div class="home-masthead">
	<h1>Contact</h1>
	<p class="home-mission">Do you have a work that should be salvaged, or a physical copy of something missing here? Corrections to a transcription or translation are just as welcome.</p>
</div>

<hr class="zag-rule" />

<div class="contact-layout">

<div class="contact-form-holder">
	<form action="https://formspree.io/f/mdopwzyo" method="POST">
		<p class="contact-field">
			<label for="contact-email" class="contact-label">Email</label>
			<input id="contact-email" type="text" name="_replyto" required />
		</p>
		<p class="contact-field">
			<label for="contact-message" class="contact-label">Message</label>
			<textarea id="contact-message" name="message" rows="7" required></textarea>
		</p>

		<div class="g-recaptcha" data-sitekey="6Lcls7oaAAAAAHE48h-DAXpojju5JLjIsHmd0IKt"></div>

		<button type="submit" class="contact-submit">Send</button>
	</form>
</div>

<aside class="contact-aside">
	<h2 class="contact-aside-title">What happens to a submission</h2>
	<p>Works offered to the archive are scanned, transcribed, and where necessary translated under the archive's <a href="/about/#editorial-method">editorial method</a>, then deposited at the <a href="https://archive.org/">Internet Archive</a> alongside their article here.</p>
	<p>Corrections are read against the scans before anything is changed. If you are reporting an error in a translation, quoting the surrounding sentence and the printed page number helps considerably.</p>
</aside>

</div>